...
public class APKCallGraph {
	...
	//directory of android.jar file
	static String androidPlatformPath = "";
	...
	public static void main(String[] args) throws Exception {
		...
		//ic3 result directory
		String ic3 = "";

		System.out.println("Start analyze apk: " + apk);
		//apk path
		String apkPath = "";
		...
		FileWriter f_writer = new FileWriter(mappingfile);
	...
	public static void generateCallGraph(String apk, String apkPath, String ic3) throws IOException, XmlPullParserException {
		...
		//put SourceAndSinks.txt path in
		...
		//put AndroidCallbacks.txt path in
		app.setCallbackFile("");
		...
		while (list.size() > 0){
			if (list.get(0).toString().contains("onCreate(android.os.Bundle)")){
				if (list.get(0).getDeclaringClass().getSuperclass().toString().contains("Activity")){
					try {
						...
						while (stmts.hasNext()){
							...
							if (s.toString().contains("setContentView")){
								String id = s.toString().substring(s.toString().lastIndexOf("(") + 1, s.toString().lastIndexOf(")"));
								actlayout.put(list.get(0).getDeclaringClass().toString(), id);
								System.out.println(list.get(0).getDeclaringClass().toString());
			...
			if (list.get(0).toString().contains("onCreateView(")){
				if (list.get(0).getDeclaringClass().getSuperclass().toString().contains("Fragment")){
					try {
						Body body = list.get(0).retrieveActiveBody();
						Iterator<Unit> stmts = body.getUnits().iterator();
						visited.put(list.get(0), true);
						while (stmts.hasNext()){
							Stmt s = (Stmt) stmts.next();
							if (s.toString().contains("inflate")){
								String id = s.toString().substring(s.toString().lastIndexOf(">(") + 2, s.toString().lastIndexOf(">(") + 12);
								actlayout.put(list.get(0).getDeclaringClass().toString(), id);
								System.out.println(list.get(0).getDeclaringClass().toString());
								System.out.println(id);
							}
		...
		//create new folder for each app to store .dot file
		File file = new File("");
		file.mkdir();
		//dot output file path
		exporter.exportGraph(jg, new FileWriter(""));
	}
	 ...
	 public static void generateSubGraphOfMethod(String apk, String method) throws IOException, SQLException {
		...
		if(!methods.containsKey(method)){
			...
			DOTExporter<MethodNode, CallEdge> exporter = new DOTExporter<MethodNode, CallEdge>(
					apkg.new MethodnodeIdProvider(), apkg.new MethodNodeNameProvider(), null);
			//exporter.exportGraph(subGraph, new FileWriter("/Users/shaoyang/Desktop/temp/" + apk + "/" + method + ".dot"));
